# Integrating @nizam-os/dashboard-sdk — multi-tenant scope
...
```ts
import { NizamDashboardClient } from '@nizam-os/dashboard-sdk';

const client = new NizamDashboardClient({ token: accessToken });
```
...
const client = new NizamDashboardClient({
  token: accessToken,
