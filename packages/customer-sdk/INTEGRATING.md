# Integrating @nizam-os/customer-sdk — multi-tenant scope
...
```ts
import { NizamCustomerClient } from '@nizam-os/customer-sdk';

const client = new NizamCustomerClient({ token: accessToken });
```
...
const client = new NizamCustomerClient({
  token: accessToken,
