# Integrating @nizam-os/carrier-sdk — multi-tenant scope
...
```ts
import { NizamCarrierClient } from '@nizam-os/carrier-sdk';

const client = new NizamCarrierClient({ token: accessToken });
```
...
const client = new NizamCarrierClient({
  token: accessToken,
