# Integrating @nizam-os/merchant-sdk — multi-tenant scope
...
```ts
import { NizamMerchantClient } from '@nizam-os/merchant-sdk';

const client = new NizamMerchantClient({ token: accessToken });
```
...
const client = new NizamMerchantClient({
  token: accessToken,
